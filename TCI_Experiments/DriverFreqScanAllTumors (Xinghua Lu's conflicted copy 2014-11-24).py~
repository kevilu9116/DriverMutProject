...
for tumor in tumorTypes:
	...
	for f in tumorFiles:
		...
		for i in range(len(gtList)):
			...
			geIndices = np.where(curMatrix.data[i] > .8)[0]
	...
	for gt, counts in sortedGTTuples:
		GEs = [x for x in mutGenesDict[gt].items() if (float(x[1])/float(counts)) > 0.2]
