...
import sys
import math
from NamedMatrixTest import NamedMatrix
import scipy as s
from util import *
...
def createComb(mutationMatrix, flag):
    ...
    for i in range(len(geneList) - 1):
        for j in range(i + 1, len(geneList)):
            ...
            if flag == AND or flag == AND_PLUS_ORIGINAL:
                outputMatrix[:, count] = gene1Vals * gene2Vals
            #'OR the two values together'
            elif flag == OR or flag == OR_PLUS_ORIGINAL:
                results = gene1Vals 
                results[np.where(gene2Vals > 0)] = 1
    ...
    if flag == AND or flag == OR:
        ...
        comboMatrix = NamedMatrix(npMatrix = outputMatrix, colnames = newColNames, rownames = mutationMatrix.getRownames())
        return mutationMatrix.concatenate(comboMatrix)        
...
#    outputFilePath = "/home/kevin/GroupDropbox/TCI/Tumor.Type.Data/BLCA/TestLogicalAnd"
    ...
    outputFilePath = "/home/kevin/GroupDropbox/TCI/chunhui.testmatrices/TestResults"
    ...
    calcTCI(mutcnaMatrixFN=mutMatrixFilePath, degMatrixFN=degMatrixFilePath, outputPath = outputFilePath,  dictGeneLength = geneLengthDict, opFlag = AND_PLUS_ORIGINAL)
